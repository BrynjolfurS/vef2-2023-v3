...
function parseLine(line: string): Omit<Course, 'id'> | null {
  const [
    id = undefined,
    title = undefined,
    lineCredits = undefined,
    lineSemester = undefined,
    lineLevel = undefined,
    lineUrl = undefined,
  ] = line.split(';');

  const formattedCredits = (lineCredits ?? '').replace(/\./g, '').replace(',', '.');
  const parsedCredits = Number.parseFloat(formattedCredits);
  const credits =
    (lineCredits ?? '').indexOf('.') < 0 &&
    !Number.isNaN(parsedCredits) &&
    formattedCredits === parsedCredits.toString()
      ? parsedCredits
      : undefined;
